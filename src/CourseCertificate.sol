...
contract CourseCertificate is ERC721, Ownable {
    ...
    // Erro personalizado para tokenId existente
    error TokenIdAlreadyExists(uint256 tokenId);
    // Erro personalizado para tokenId não existente
    ...
    constructor(address institutionAddress, string memory name, string memory symbol)
        ERC721(name, symbol)
    ...
    function mintCertificate(
        ...
        onlyOwner
    {
        require(bytes(studentName).length != 0 && bytes(studentName).length <= 100, "Invalid student name");
        require(bytes(courseName).length != 0 && bytes(courseName).length <= 100, "Invalid course name");
        require(bytes(institutionName).length != 0 && bytes(institutionName).length <= 100, "Invalid institution name");
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory json = string(
            abi.encodePacked(
                ...
                Strings.toString(data.completionDate),
                '"}',
                "]}"
            )
        );
